---
title: "Some helpful tips for formatting tables"
...
# Reproduce changes from Tutorials 1 and 2:
...
# Coerce room_type from character to factor
...
# Print as kable table with formatting
...
table_8 <- listings %>%
  ...
  kable(caption = "Table 8. Property Price ($) by Property Type", format= "html") %>%
    kable_styling(full_width = TRUE, position = "left")
...
anova_model_1 <- listings %>%
  aov(log(price) ~ room_type, data = .)  # Store the aov model first
...
kable(t_test_table, caption = "Table 12. T-test Log Price vs. Room_type. Source: Inside AirBnB.", format = "html") %>%
  kable_styling(full_width = TRUE, position = "left")
...
cor_table <- tidy(cor_result) %>%
 ...
 rename(
  ...
  select(
    ...
    `Correlation (r)`, 
    `t-statistic`, 
    `Lower CI (95%)`,  # Or use `conf.low` if not renamed
    `Upper CI (95%)`, 
    `p-value`
  ) %>%
...
htmlreg(list(m4_log_1, m4_log_2),
        stars = 0.05,
          digits = 2, 
          caption = "Table 12. Regression results. Dependent variable: Price(logged)",
          caption.above = TRUE,
